...
smd({
    ...
    async({reply, chat ,isGroup, sender,error}) => {
      try{
        ...
        const daily  = await eco.daily(sender, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 500); //give 500 for daily, can be changed
        if (daily.cd) {
...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳")
       await eco.deduct(users, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", balance.wallet);
  ...
  try{
   ...
   switch (value) {
       ...
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 100);
         const add1 = eco.giveCapacity(user,"𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 1000);
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 1000);
         const add2 = eco.giveCapacity(user, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 100000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 10000);
          const add3 = eco.giveCapacity(user, "𝙺𝙸𝙽𝙶 𝙼𝙰𝙻𝚅𝙸𝙽 𝙼𝙳", 10000000);
