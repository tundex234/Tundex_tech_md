...
async(message, match) => {
 try{
   ...
   message.react("🔍")
         if (!match) return message.reply(`Give me a user name like ${prefix}github malvinking`)
...
smd({
    ...
    async(message, text) => {
 if(!text) return await message.reply('Give Me Number without + sign. Example: .iswa 2637147578xx')
        var inputnumber = text.split(" ")[0]
        if (!inputnumber.includes('x')) return message.reply(`*You did not add x*\nExample: iswa 2637147578xx  \n ${Config.caption}`)
...
smd({
    ...
    async(message, text) => {
if(!text) return await message.reply('Give Me Number without + sign. Example: .nowa 2637147578xx')
const inputNumber = text.split(" ")[0]
if (!inputNumber.includes('x')) return message.reply(`*You did not add x in number.*\nExample: ${prefix}nowa 2637147578xx  \n ${Config.caption}`)
